...
INSERT INTO upload_tasks(
    ...
    target_path, target_display_path, status, progress, uploaded_bytes, speed_bps,
    total_bytes, message, error, result_json, resume_data_json, queue_order,
    created_at, updated_at, local_path, conflict_policy
) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
ON CONFLICT(task_id) DO UPDATE SET
    ...
    conflict_policy=excluded.conflict_policy`,
		...
		rec.TargetPath, rec.TargetDisplayPath, rec.Status, rec.Progress, rec.UploadedBytes, rec.SpeedBytesPerSecond,
		rec.TotalBytes, rec.Message, rec.Error, rec.ResultJSON, rec.ResumeDataJSON, rec.QueueOrder,
